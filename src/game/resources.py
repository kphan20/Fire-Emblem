...
#starting_screen = pyglet.resource.animation('menu/titlescreen.gif')
...
frames = [pyglet.resource.image('animation/eliwood attack/' + file_name) for file_name in eliwood_animations]
eliwood_animation = pyglet.image.Animation.from_image_sequence(frames, duration=0.1, loop=True)

tile_selector = listdir('../resources/animation/tile selector')
frames = [pyglet.resource.image('animation/tile selector/' + file_name) for file_name in tile_selector]
tile_selector_animation = pyglet.image.Animation.from_image_sequence(frames, duration=0.2, loop=True)

path_arrows = listdir('../resources/gameplayui/patharrows')
path_arrows_dict = dict((name.split('.')[0], pyglet.resource.image(f'gameplayui/patharrows/{name}')) for name in path_arrows)
